...
<template>
		<div class="border-bottom ml-1" style="width: 100%;" >
			<div v-for="(recommend,index) in recommendList" :key="index" class="border-bottom">
				<v-list>
					<span class="font-weight-black title ml-5">{{ recommend.target.question.title }}</span>
					<div v-if="!flag[index]" >
						<!-- 带有视频显示的四行文本内容区 -->
						<div class="hy-recommend-card-content d-flex pl-3 " v-if="recommend.target.thumbnail">
								<div class="col-4">
									<img :src="recommend.target.thumbnail" style="border-radius: 3%;">
								</div>
								<div class="col-8">
					...
					<div class="pl-5 pr-5 mt-1" v-if="flag[index]">
						 <v-avatar tile size=30 style="border-radius: 10%;">
						      <img
						        :src="recommend.target.author.avatar_url"
						      >
						    </v-avatar>
					...
					<div :class="flag[index]?'hy-recommend-hideLine pos-sti hy-recommend-hideLine-more pl-3' :'hy-recommend-hideLine   pl-5'">
						<v-btn  class="ma-1 blue--text" depressed color="#d9edff" width=100 height=33>
							...
							赞同{{recommend.target.voteup_count }}
							</v-btn>
						...
						<span class="mt-5 ml-5 font-14 light-grey">
						...
						<span class="mt-5 offset-l-2 font-14 light-grey link" v-if="flag[index]" @click="getPackUp(index)">
							收起
